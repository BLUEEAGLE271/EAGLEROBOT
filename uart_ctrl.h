...
extern float targetLinearVelocity = 0.0;
extern float targetAngularVelocity = 0.0;
extern bool isMoving = false;
 // Flag to enable/disable PID loop
void jsonCmdReceiveHandler(){
	...
	case CMD_VELOCITY_CTRL:	if (jsonCmdReceive.containsKey("T") &&
														jsonCmdReceive.containsKey("L") &&
														jsonCmdReceive.containsKey("A")){
													if (jsonCmdReceive["L"].is<float>() &&
															jsonCmdReceive["A"].is<float>()){
														heartbeatStopFlag = false;
														lastCmdRecvTime = millis();
														targetLinearVelocity = jsonCmdReceive["L"];
            								targetAngularVelocity = jsonCmdReceive["A"];
            								isMoving = true;
														Serial.println("Running");
													}
												} break;

	case CMD_SET_ANGULAR_MOTOR_PID:
												pidZ.setGains(
												jsonCmdReceive["P"],
												jsonCmdReceive["I"],
												jsonCmdReceive["D"]);
