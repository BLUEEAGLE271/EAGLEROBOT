...
void setGoalSpeed(float linear, float angular) {
  // setpoint_cmd_recv = millis();
    usePIDCompute = false;
    

    float outputZ   = pidZ.compute(angular, gz*0.01745);
    
    float turnLinearVel = (angular + outputZ )*TRACK_WIDTH/2.0;

    if ( (turnLinearVel)> 1) turnLinearVel = 1;
    if ( (turnLinearVel)< -1) turnLinearVel = -1;

    float availableLinear = 1 - abs(turnLinearVel);
    if (linear > availableLinear)  linear = availableLinear;
    if (linear < -availableLinear) linear = -availableLinear;
    Serial.println(turnLinearVel);
    leftCtrl((linear - turnLinearVel) * 255 * spd_rate_A);
    rightCtrl((linear + turnLinearVel) * 255 * spd_rate_B);
  }
